...
const app = createApp({
  ...
  methods: {
    ...
    agregarFavorito(maps) {
      ...
      alert(maps.displayName + " was saved successfully");
    },
    eliminarFavorito(maps) {
      let index = this.mapsFavorites.findIndex((res) => res.uuid == maps.uuid);
      if (index !== -1) {
  ...
  computed: {
    filterData() {
      const filterText = this.mapsBk.filter((maps) =>
        maps.displayName.toLowerCase().includes(this.textSearch.toLowerCase())
      );
      this.maps = filterText;

      this.maps = filterText.filter(
        (maps) => this.regionSelection >= maps.callouts.length
      );
    },
  },
}).mount("#app");
